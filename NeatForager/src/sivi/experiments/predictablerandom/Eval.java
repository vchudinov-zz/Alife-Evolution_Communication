...
public class Eval extends MasterEvaluator {
		
	
	public Eval(Properties prop) {
		super(prop);
	}
	...
	public boolean evaluate(Organism organism) {
		...
		random = new Random(seed); //Look in MasterEvaluator
	...
	/**
	 * Fitness is based on amount of food collected - amount of poison collected
	 * @param sim
	 * @return
	 */
	private double calculateFitness(Simulator sim){
		int collectedFood = 0;
		int collectedPoison = 0;
		int foodStart = numFoodStart * energyStartFood;
		double totalFitness = 0; 
		for (Entity e : sim.getWorld().getEntitiesOnMap()){
			if (e.getType() == TYPE.AGENT){ 
				MasterAgent a = (MasterAgent) e;
				collectedFood += a.getCollectedFood();
				collectedPoison += a.getCollectedPoison();
			}
		}
		
		totalFitness = (collectedFood - collectedPoison) / (double) foodStart;
