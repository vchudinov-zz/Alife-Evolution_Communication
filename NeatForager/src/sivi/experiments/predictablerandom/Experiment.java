package sivi.experiments.predictablerandom;
...
public class Experiment extends MasterExperiment{
	
	private final String KEY_MIN_GENERATION_WIN_NUMBER = "min_generation_win_number";
	
	final String delimiter = "/";
	private int MIN_GENERATION_WIN_NUMBER;
	//Is set in the code
	String championFolder;
	String epochInfoFolder;
	
	public Experiment(String propertiesFile) {
		super(propertiesFile);
	}
	
	@Override
	/**
	 * Experiment doesn't end earlier if a winner is found
	 */
	public void runExperiment(){
		...
		for (int i = 0; i < numExpRuns; i++){
			...
			evaluator.setRandomizerSeed(seed);
			saveSeed(seed, i);
			...
			//Create ancestor genome
				...
				do {
					...
					if (numberOfWinners > 0){
						generationsWithWinners++;
					} else {
						generationsWithWinners = 0;
					}
					
					gen++;
				} while (gen <= maxGenerations && generationsWithWinners < MIN_GENERATION_WIN_NUMBER);
	...
	private void printBest(Iterable<Organism> list, int generation)
		...
		double fitnessToPrint = maxFitnessThisGeneration;
	...
	/**
	 * Prints the given seed to a file. Useable if we want to recreate an experiment
	 * @param generation
	 * @param fitness
	 */
	private void saveSeed(long seed, int experimentNumber) {
		String xNameFile = outputFolder + delimiter + "seedlist.csv";
		...
		try {
			PrintWriter out = new PrintWriter(new BufferedWriter(
					new FileWriter(xNameFile, true)));
			out.println(experimentNumber + ";" + seed);
			out.close();
		} catch (IOException e) {
			// oh noes!
		}
	...
	protected boolean loadSpecificExperimentProperties() {
		String s = expProp.getProperty(KEY_MIN_GENERATION_WIN_NUMBER);
		MIN_GENERATION_WIN_NUMBER = Integer.parseInt(s);
