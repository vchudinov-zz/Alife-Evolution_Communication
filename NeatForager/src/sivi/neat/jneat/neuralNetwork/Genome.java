...
public class Genome extends Neat {
	...
	public void mutate_link_weight(double power, double rate, int mutation_type) {
		...
		while (itr_gene.hasNext()) {
			...
			//Making sure gene weights are within bounds 
			//STC 11-04-2014
			if (_gene.getLnk().weight > 5){
				_gene.getLnk().weight = 5;
			} 
			if (_gene.getLnk().weight < -5){
				_gene.getLnk().weight = -5;
			}
	...
	public double compatibility(Genome g) {
		...
		for (j = 0; j < max_genome_size; j++) {
			...
			if (j1 >= size1) {
				...
				if (p1innov == p2innov) {
					num_matching += 1.0;
					mut_diff = Math.abs(_gene1.getMutation_num()
							- _gene2.getMutation_num());
	...
	/**
	 * STC Verifies that the genome is created correctly
	 * 
	 * @return
	 */
	public boolean verify() {
		...
		// control if nodes in the genes are defined and are the same nodes as
		// in the nodes list
		itr_gene = genes.iterator();
		while (itr_gene.hasNext()) {
	 ...
	 * 
	 * @param g
	 *            the genome (dad) to mate this genome (mom) with
	 * @param genomeid
	 *            Id for the new baby
	 * @param fitness1
	 *            The fitness of mom (this genome)
	 * @param fitness2
	 *            The fitness of dad (The other genome)
	 * @return
	 */
	public Genome mate_multipoint(Genome g, int genomeid, double fitness1,
		...
		/************************************************
		 * Added 05-11-2013 Author: STC Based on code in the FAQ on the NEAT
		 * webpage http://www.cs.ucf.edu/~kstanley/neat.html#FAQ1 /
		 *************************************************/
		...
		for (NNode n : nodes) {
			...
			if (nodeLabel == NeatConstant.INPUT
				...
				NNode newNode = new NNode(n,
						newtraits.elementAt(nodeTraitNumber));
   ...
   */
	...
	/**
	 * Adds new gene to the network. Will never add a gene where a sensor is the
	 * out-node
	 * 
	 * @param pop
	 * @param tries
		...
		if (found) {
			...
			while (!done) {
				...
				if (!itr_innovation.hasNext()) {
					...
					// Add the innovation
					pop.getInnovations().add(
							new Innovation(thenode1.node_id, thenode2.node_id,
									curr_innov, new_weight, traitnum));
					done = true;
	...
	/**
	 * STC Adds a new node to the network. The node is added between two nodes
	 * already connected by an active gene. New nodes are never created when a
	 * BIAS node is the in-node if the gene
	 * 
	 * @param pop
	 * @return
	 */
	public boolean mutate_add_node(Population pop) {
		...
		if (genes.size() < 15) {
			...
			step2 = false;
			// Find the first enabled gene where the in node is not a BIAS node
			// //STC
			...
			// From the first enabled gene with a in-node which is not a
			// BIAS-node it loops through
			// the genes and stops at the first gene with a non-bias in-node
			...
			// Tests 20 random genes and chooses the first one which is enabled
			// and has a non-bias in-node
		...
		while (!done) {
			...
			if (!itr_innovation.hasNext()) {
				...
				pop.getInnovations().add(
						new Innovation(in_node.node_id, out_node.node_id,
								gene_innov1, gene_innov2, new_node.node_id,
								_gene.innovation_num));
				done = true;
			}
	...
	public void mutate_add_sensor(Population pop) {
		...
		NNode sensor = (NNode) sensors.elementAt(NeatConstant.myRandom
				.nextInt(sensors.size()));
		...
		for (int i = 0; i < outputs.size(); i++) {
			...
			for (int j = 0; j < genes.size(); j++) {
				Gene gene = (Gene) genes.elementAt(j);
				if (gene.lnk.in_node == sensor && gene.lnk.out_node == output)
					found = true;
			}
			...
			if (!found) {
				...
				while (!done) {
					if (!innovations.hasNext()) {
						int traitNum = NeatRoutine
								.randint(0, traits.size() - 1);
						double newWeight = NeatRoutine.randposneg()
								* NeatRoutine.randfloat() * 10.0;

						double currInnov = pop
								.getCurr_innov_num_and_increment();
						Gene gene = new Gene(
								(Trait) traits.elementAt(traitNum), newWeight,
								sensor, output, false, currInnov, newWeight);
						genes.add(gene);

						pop.getInnovations().add(
								new Innovation(sensor.node_id, output.node_id,
										currInnov, newWeight, traitNum));
						...
						if (innov.innovation_type == NeatConstant.NEWLINK
							...
							Gene gene = new Gene(
									(Trait) traits
											.elementAt(innov.new_traitnum),
									innov.new_weight, sensor, output, false,
									innov.innovation_num1, 0);
							genes.add(gene);
	...
	/**
	 * Creates a genome based on
	 * 
	 * @param id
	 *            - ID of the genome
	 * @param xFile
	 *            - Filename of the genome
	 */
